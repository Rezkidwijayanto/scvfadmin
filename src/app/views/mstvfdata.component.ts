...
declare var $ :any ;
@Component({
  selector: 'app-mstvfdata',
  templateUrl: './mstvfdata.component.html' 
})
export class MstvfdataComponent implements OnInit {
  plant:any="PEM";
  month:any="";
  datain:any=[];
  flag:any=false; 
  ...
  createRange = paginationFunction.createRange;
   constructor(private fb: FormBuilder,private dataserv:DataService) {
    ...
    getvflog(){ 
      ...
      this.dataserv.sendPostRequest(StandardFunction.apilink+"api/getvflog",param).subscribe((data:any)=>{
  ...
  execute(){
    var filechk:any=0;
    var  url = "api/vf_cycle";
    var filename:any = [];
    if (this.file[0]) { 
      const formData = new FormData(); 
      var name: string = "" 
      name = this.plant + '_' + "Vf_Cycle"+"-"+this.month.replace("-","")+".xlsx"; 
      filename.push(name);  
